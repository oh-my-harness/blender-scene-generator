...
| **LLM API** | 支持 OpenAI 兼容 API 和 Anthropic。OpenAI 路径：`OPENAI_API_KEY`（可设置 `OPENAI_API_BASE`、`OPENAI_MODEL`）。Anthropic 路径：设 `LLM_PROVIDER=anthropic`，配 `ANTHROPIC_API_KEY`（可设置 `ANTHROPIC_MODEL`、`ANTHROPIC_API_BASE`）。 |
...
依赖没装。运行 `pip install -r blender_scene/requirements.txt --find-links https://github.com/oh-my-harness/llm-harness-py-wheels/releases/expanded_assets/v0.3.0`。确认 Python 版本是 3.12。

**LLM 调用失败 / 超时**
检查 API key 与 base URL 是否正确、网络是否能访问该端点。OpenAI 路径看 `OPENAI_API_KEY`/`OPENAI_API_BASE`；Anthropic 路径看 `ANTHROPIC_API_KEY`/`ANTHROPIC_API_BASE`。`.env` 里可切换不同 provider 对比测试。
